...
class Dashboard_model extends CI_Model{
	...
	public function get_records($from){
		...
		if(isset($from) && $from == 0){
			...
			$sql = 'SELECT month(a.[Date]),year(a.[Date]) as [cYear], sum([SUM$Cost Amount])*-1 as [Sale]
  FROM [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Trans_ Sales Entry$VSIFT$1] as a
  JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item Vendor] as I ON I.[Item No_] = a.[Item No_]
  where I.[Vendor No_]=\''.$vendor.'\'  and Year(a.[Date]) = YEAR(CURRENT_TIMESTAMP)
  group by month(a.Date),year(a.[Date])
  order by month(a.Date),year(a.[Date])';
			
			/*SELECT Month(T.Date),Year(T.Date) as [cYear],
  ...
  FROM [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Trans_ Sales Entry$VSIFT$1] as a
  JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item Vendor] as I ON I.[Item No_] = a.[Item No_]
  where I.[Vendor No_]=\''.$vendor.'\'  and Year(a.[Date]) = YEAR(CURRENT_TIMESTAMP)-1
  group by month(a.Date),year(a.[Date])
  order by month(a.Date),year(a.[Date])';
	...
	public function get_category_records($category,$from,$to,$period){
		...
		if($period == 0){ 
			...
			$sql = 'SELECT TOP 10 SUM([SUM$Net Amount]*-1) Sale,I.[Vendor No_] as vnumber, V.Name,100* SUM([SUM$Net Amount]*-1) / (SUM(SUM([SUM$Net Amount]*-1)) OVER ()) "MarketShare" 
				FROM [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Trans_ Sales Entry$VSIFT$1] AS E
				JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item] AS I ON I.No_ = E.[Item No_] 
				JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Vendor] AS V ON V.No_ = I.[Vendor No_] 
				WHERE I.[Product Group Code] = \''.$category.'\' AND ([Date]) >= \''.$from.'\' AND ([Date]) <= \''.$to.'\' AND Year([Date]) = YEAR(CURRENT_TIMESTAMP)-1
				GROUP BY I.[Vendor No_],I.[Item Category Code],V.Name
				ORDER BY Sale desc';
	...
	public function get_vendor_category_records($category,$from,$to,$period){
		...
		if($period == 0){ 
			...
			$sql = 'SELECT TOP 10 SUM([SUM$Net Amount]*-1) Sale,I.[Vendor No_] as vnumber, 100* SUM([SUM$Net Amount]*-1) / (SUM(SUM([SUM$Net Amount]*-1)) OVER ()) "MarketShare" 
				FROM [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Trans_ Sales Entry$VSIFT$1] AS E
				JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item] AS I ON I.No_ = E.[Item No_] 
				WHERE  I.[Product Group Code] = \''.$category.'\' AND ([Date]) >= \''.$from.'\' AND ([Date]) <= \''.$to.'\' AND Year([Date]) = YEAR(CURRENT_TIMESTAMP)-1
				GROUP BY I.[Vendor No_], I.[Product Group Code]
				ORDER BY Sale desc';
		}else{
