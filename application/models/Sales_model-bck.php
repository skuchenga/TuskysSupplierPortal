...
JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item] as I ON I.[No_] = a.[Item No_]
JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Store] as S ON a.[Store No_] = S.[No_]
...
where a.Date>=\''.$from.'\' and a.Date<=\''.$to.'\' and a.[Item No_] in 
(select a.[Item No_]  from [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item Vendor] as a where a.[Vendor No_] = \''.$vendor.'\')
...
where a.Date>=\''.$from.'\' and a.Date<=\''.$to.'\' and a.[Item No_] in 
(select a.[Item No_]  from [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item Vendor] as a where a.[Vendor No_] = \''.$vendor.'\')
group by [Store No_],[Item No_],I.[Description],C.[Description],G.Description,S.Name';
		}
		//echo $sql;exit;
		$query = $this->db->query($sql);
	...
	public function get_records_query($from, $to){
      ...
      ,sum([SUM$Cost Amount]*-1) as [Price],
      I.[Description] as Description,C.[Description] as Category,G.Description as [Product Group],S.Name
  FROM [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Trans_ Sales Entry$VSIFT$1] as a   
JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Item] as I ON I.[No_] = a.[Item No_]
JOIN [TuskysDB_Live].[dbo].[Tusker Mattresses Ltd$Store] as S ON a.[Store No_] = S.[No_]
